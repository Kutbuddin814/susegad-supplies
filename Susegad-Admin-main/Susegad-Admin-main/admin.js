document.addEventListener('DOMContentLoaded', () => {
  ...
  tabLinks.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      document.querySelectorAll('.tab-content-area').forEach(div => {
      ...
      // 🌟 CRITICAL FIX: CALL THE LOAD FUNCTION ON CLICK 🌟
      if (tabId === 'products') {
        allProductsCache = [];
        loadProducts();
      } else if (tabId === 'categories') { // ⬅️ Categories Clicked
        loadCategories();
      } else if (tabId === 'orders') {     // ⬅️ Orders Clicked
        loadOrders();
      }
    });
  });
