...
  const contentAreas = document.querySelectorAll('.tab-content-area'); // All content sections
...
    } catch (e) {
      cTbody.innerHTML = '<tr><td colspan="1" style="text-align:center; padding: 20px; color: red;">Failed to load categories.</td></tr>';
      console.error(e);
    }
  }
...
      }).join("");
    } catch (e) {
      oTbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding: 20px; color: red;">Failed to load orders.</td></tr>';
      console.error(e);
    }
  }
...
      contentAreas.forEach(div => { // Use the cached contentAreas NodeList
        div.classList.add("hidden");
      });
      // CRITICAL FIX: The ID of the target section now correctly matches the pattern
      const contentDiv = document.getElementById(`tab-${tabId}-content`); 
      if (contentDiv) {
        contentDiv.classList.remove("hidden");
      }

      // 3. Call the correct load function 
      if (tabId === 'products') {
...
        if (res.ok) {
          // showToast(`Status updated to ${newStatus}.`, 'success'); // Using toast instead of alert
          loadOrders();
        } else {
          const data = await res.json();
          showToast(`Failed: ${data.message || res.status}`, 'error');
        }
      } catch (error) {
        console.error("Status update failed:", error);
        showToast("Server connection failed during status update.", 'error');
      }
    }
  });
...
  // CRITICAL FIX: Hide all sections first to ensure only the active one is shown
...
    const contentDiv = document.getElementById(`tab-${initialTabId}-content`);
    if (contentDiv) {
      contentDiv.classList.remove("hidden");
    }
